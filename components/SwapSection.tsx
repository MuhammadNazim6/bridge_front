...
function SwapSection({ srcToken, destToken }: { srcToken: IToken, destToken: IToken }) {
  ...
  const handleGetQuote = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:3002/api/getQuote`, {
        params: {
          ...
          <DrawerHeader className="h-96">
            ...
            <div className="md:flex items-center overflow-x-auto custom-scrollbar">
              {quotes?.length && quotes.map((quote) => {
                return (
                  <div className="h-52 shadow-lg bg-accent/55 w-1/4 rounded-r-lg min-w-[250px] active:scale-[.98] active:duration-75 transition-all hover:scale-[1.02] ease-in-out p-6 m-1 cursor-pointer text-xs">
